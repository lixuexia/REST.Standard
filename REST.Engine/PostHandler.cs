    ...
    public class PostHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                {
                    RESTParams.Add(item.ToLower(), context.Request.Form[item]);
                }
                #region 检测服务方法
                ...
                #region 数据检测
                #region 请求方法检测
                if ("|GET|PUT|DELETE|".Contains("|" + context.Request.HttpMethod + "|"))
                {
                    context.Response.StatusCode = 400;
                    context.Response.StatusDescription = "错误请求:请求方法应为POST";
                    OutResponse(context, RESTResponse);
                    return;
                }
                #endregion
                ...
                //{
                //    RESTResponse.IsSuccess = 0;
                //    RESTResponse.ErrorMessage = "Error_" + TimeStampresult + "：" + Global.ErrorMessage[TimeStampresult];
                //    OutResponse(context, RESTResponse);
                //    return;
                //}
                #endregion
            ...
            catch (Exception ex)
            {
                ...
                RESTResponse.ErrorCode = 0;
                RESTResponse.ErrorMsg = "Error_9001：" + Global.ErrorMessage["9000"] + "\r\n" + ex.StackTrace + "\r\n" + ex.Message;
                OutResponse(context, RESTResponse);
            }
        }
